...
bool udpSend(sockaddr_in servaddr, const uint8_t *packet, uint16_t length) {
  ...
  int ret =
      sendto(fd, packet, length, 0, (sockaddr *)&servaddr, sizeof(servaddr));
  close(fd);
  return ret == length;
}

uint16_t htoartnet_le(uint16_t host) { return host; }

uint16_t htoartnet_be(uint16_t host) { return __builtin_bswap16(host); }
...
ArtnetConnection::ArtnetConnection(ArtnetTarget *targetConfiguration) {
  ...
  inet_aton(this->targetConfiguration->address.c_str(),
            &this->servaddr.sin_addr);

  printf("ArtnetSender: Preparing connecting to \"%s\" port %d...\n",
         this->targetConfiguration->address.c_str(),
         this->targetConfiguration->port);
}

ArtnetConnection::~ArtnetConnection() {}

void ArtnetConnection::sendUniverse(uint8_t sequence, uint8_t universe,
                                    uint16_t length, uint8_t *channels) {
  ...
  udpSend(this->servaddr, (const uint8_t *)dmxpacket,
          sizeof(ArtnetDmxHeader) + length);
}
...
void ArtnetConnection::sendSync() {
  ...
  udpSend(this->servaddr, (const uint8_t *)&temppacket,
          sizeof(ArtnetSyncHeader));
}

void ArtnetConnection::handleVideoFrame(uint16_t frame, uint8_t *pixels,
  ...
  for (int u = 0; u < this->targetConfiguration->universes.size(); u++) {
    ...
    if (uni->type == "dmx512row") {
      sendUniverse(frame, uni->universe, uni->length,
                   pixels + 3 * (uni->y * 256 + uni->x));
    }
  }
}
...
void ArtnetSender::handleVideoFrame(uint16_t frame, uint8_t *pixels,
                                    uint16_t width, uint16_t height) {
  // Send dmx frames
