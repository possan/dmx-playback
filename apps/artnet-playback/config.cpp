...
bool ArtnetTarget::parse(nlohmann::json &node) {
    ...
    if (node.contains("universes")) {
        for (nlohmann::json::iterator it = node["universes"].begin(); it != node["universes"].end(); ++it) {
            ArtnetUniverse o;
            if (o.parse(*it)) {
...
bool Configuration::parse(nlohmann::json &node) {
    ...
    if (node.contains("events")) {
        for (nlohmann::json::iterator it = node["events"].begin(); it != node["events"].end(); ++it) {
            TimedEvent o;
            if (o.parse(*it)) {
    ...
    if (node.contains("targets")) {
        for (nlohmann::json::iterator it = node["targets"].begin(); it != node["targets"].end(); ++it) {
            ArtnetTarget o;
            if (o.parse(*it)) {
